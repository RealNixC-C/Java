...
public class FileExample {
	...
	public static void main(String[] args) {
		...
		// 경로 구분자
		// Windows: \(역슬래시) + 호환성과 편의성을 위해 /를 입력해도 \로 자동변환처리 
		...
		File file3 = new File("C:\\Temp\\file3.txt"); // 문자열에서는 \\로 이스케이프 필요
...
//		if (!dirTest.exists()) dirTest.mkdir(); // 경로상 맨뒤 폴더만 생성(앞에 상위 폴더가 있어야함)
		...
		if (temp.exists() && temp.isDirectory()) {
			...
			for (File file : contents) {
				...
				// 날짜 형식으로 바꾸기 위해 SimpeDateFormat 활용
				System.out.print(sdf.format(new Date(file.lastModified())));
				
				// java.time API 사용 시
				// long -> Instant -> ZonedDateTime(또는 LocalDateTime)
//				ZonedDateTime dateTime = Instant.ofEpochMilli(file.lastModified()) // 에포크 기준 밀리초 -> Instant로 반환
//												.atZone(ZoneId.systemDefault()); // 한국이니까 Asia/Seoul(UTC+9)
