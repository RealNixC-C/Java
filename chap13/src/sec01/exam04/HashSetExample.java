...
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		// Set 특징
		// 수학의 집합에 비유
		// 순서 없이 데이터를 저장(저장된 요소들을 순서대로 꺼낼 수없음)
...
//		Set<String> set = new HashSet<String>();
		Set<String> set = new HashSet<>(); // 생략하면 왼쪽 Set에 저장된 타입을 따라 감(타입 추론)
		...
		while (iterator.hasNext()) { // 가져올 객체가 있으면 true, 없으면 false 반환
			String element = iterator.next(); // 컬렉션에서 1개의 객체를 가져옴
			// 텍스트 커서는 첫 요소 이전 위치에 있음, next() 호출하면 커서가 다음 위치로 이동하면서 그 요소를 반환함
...
//			iterator.remove();
//			}
			// 참고: remove()는 next() 호출 이후에만 호출가능하며, 한 번만 가능 
		} 		
...
//		class1.addAll(class2); // 합한값이 class로 반영됨
...
//		System.out.println("차집합: " + class1);
		...
		// Set -> List 변환
		// 생성자의 매개값으로 fruits를 넣음으로 ArrayList로 변환 // 메소드에 오버라이딩 되어있음
		...
		// 필요하면 정렬된 순서를 유지하는 Set으로 다시 만들기 (다시 HashSet으로 돌릴시 뒤섞이므로 의미가없음)
		Set<String> sortedSet = new LinkedHashSet<String>(fruitsList);
		System.out.println(sortedSet);
		
		
		// ArrayList의 요소에서 중복을 제거하는 방법 > 간단하게 HashSet으로 변환
		...
		// HashSet으로 변환하면서 자동으로 중복제거됨
		Set<String> javaClass = new HashSet<String>(list); // 중복 제거 완료
		System.out.println(javaClass);
		
		// 필요 시 다시 List로 변환
		list = new ArrayList<String>(javaClass);
		System.out.println(list);
